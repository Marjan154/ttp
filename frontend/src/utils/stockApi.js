...
export let getAllStocks = async searchval => {
  let url = `https://cloud.iexapis.com/stable/search/${searchval}?token=${skey}`;
...
export let getStockAllPrices = async symbols => {
  ...
  let url = `https://cloud.iexapis.com/stable/stock/market/batch?symbols=${symbolsS}&types=price,quote,ohlc&token=${skey}`;
  let sdata = await axios.get(url).then(data => data.data);
  return sdata;
};

export let getStockPrice = async symbol => {
  let url = `https://cloud.iexapis.com/stable/stock/${symbol}/quote/latestPrice?token=${skey}`;
